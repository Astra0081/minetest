	...
	const GUIChatConsole *chat_console, float dtime)
{
	...
	bool overriden = g_touchscreengui && g_touchscreengui->isStatusTextOverriden();
	if (overriden) {
		guitext_status = g_touchscreengui->getStatusText();
		m_guitext_status->setVisible(false);
	} else {
		guitext_status = m_guitext_status;
		if (g_touchscreengui)
			g_touchscreengui->getStatusText()->setVisible(false);
	}
