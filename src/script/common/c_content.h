...
void push_pointed_thing(lua_State *L, const PointedThing &pointed,
		bool csm = false, bool hitpoint = false);
...
bool read_hud_change (lua_State *L, HudElementStat &stat,
		HudElement *elem, void **value);

void push_collision_move_result(lua_State *L,
		const collisionMoveResult &res);

void push_mod_spec(lua_State *L, const ModSpec &spec,
